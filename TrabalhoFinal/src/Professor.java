...
public class Professor extends Dados_Pessoa  {
    ...
    public boolean isInicioContratoValido() {
        try {
             return verificaData(this.inicioContrato) && verificaDataContrato(this.inicioContrato);
        } catch (DataInvalidaException e) {
            return false;
        }
    }
    ...
    public ArrayList<Turma> getMinistradas() {
    ...
    private boolean verificaDataContrato(String data) {
        ...
        try {
            ...
            if(dataContrato.isAfter(hoje)) {
                throw new DataInvalidaException("Data de início do contrato não pode estar no futuro.");
            }
